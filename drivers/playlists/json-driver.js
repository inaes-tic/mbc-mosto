...
function json_driver() {
    ...
    json_driver.prototype.start = function() {
        console.log("json-driver: [INFO] Start watching playlists dir " + config.playlists.to_read) ;
        watchr.watch({
            paths: [config.playlists.to_read],
            listeners: {
                error: function(err){
                    console.log("json-driver: [ERROR] Error while watching playlists dir " + config.playlists.to_read, err);
                },
                watching: function(err, watcherInstance, isWatching){
                    if (err) {
                        console.log("json-driver: [ERROR] Error while watching playlists dir " + config.playlists.to_read, err);
                    } else {
                        console.log("json-driver: [INFO] Finish watching playlists dir " + config.playlists.to_read);
                    }
                },
